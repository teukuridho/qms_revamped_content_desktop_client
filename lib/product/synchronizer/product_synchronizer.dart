...
import 'product_synchronizer_logger.dart';
...
class ProductSynchronizer {
  ...
  final ProductDownloaderBase _downloader;
  final ProductDeleterBase _deleter;
  ...
  ProductSynchronizer({
    ...
    required ProductDownloaderBase downloader,
    required ProductDeleterBase deleter,
  ...
  void _onAuthLoggedInEvent(AuthLoggedInEvent event) {
    ...
    ProductSynchronizerLogger.info(
      'AuthLoggedInEvent matched. Starting product SSE subscribe loop (serviceName=$serviceName tag=$tag)',
    );
    _startSubscribeLoop();
  }
  ...
  Future<void> _runSubscribeLoop(int gen) async {
    ...
    while (!_disposed && gen == _generation) {
      try {
        ...
        ProductSynchronizerLogger.warn(
          'Product SSE stream ended; retrying in ${retryDelay.inSeconds}s (serviceName=$serviceName tag=$tag)',
        );
      } catch (e, st) {
        ...
        ProductSynchronizerLogger.error(
          'Product subscribe failed; retrying in ${retryDelay.inSeconds}s (serviceName=$serviceName tag=$tag)',
          error: e,
          stackTrace: st,
        );
      } finally {
  ...
  Future<void> _subscribeOnce(int gen) async {
    ...
    ProductSynchronizerLogger.info(
      'Subscribing product SSE: $subscribeUri (serviceName=$serviceName tag=$tag)',
    );
    ...
    final client = _sseClientFactory(
      SseClientOptions(
        ...
        logger: (level, message, {error, stackTrace}) {
          final prefix = 'SSE(product serviceName=$serviceName tag=$tag)';
          switch (level) {
            case SseClientLogLevel.debug:
              ProductSynchronizerLogger.debug('$prefix $message');
              break;
            case SseClientLogLevel.info:
              ProductSynchronizerLogger.info('$prefix $message');
              break;
            case SseClientLogLevel.warn:
              ProductSynchronizerLogger.warn('$prefix $message');
              break;
          ...
          if (level == SseClientLogLevel.info &&
            ...
            if (seenConnectedLog) {
              ProductSynchronizerLogger.warn(
                'Product SSE reconnected; forcing full refresh '
        ...
        sseIncrementalMismatchCallback: (mismatch) {
          ProductSynchronizerLogger.warn(
            'Product SSE incremental id mismatch (serviceName=$serviceName tag=$tag): $mismatch',
          );
  ...
  Future<void> _processCreated(int gen, SseClientBase client) async {
    await for (final raw in client.listenJson(createdFieldKey)) {
      if (_disposed || gen != _generation) break;
      ProductSynchronizerLogger.debug('Created raw payload: $raw');

      ProductCreatedEventDto? dto;
      try {
        dto = ProductCreatedEventDto.fromJson(raw);
      } catch (e, st) {
        ProductSynchronizerLogger.error(
      ...
      if (product.tag != tag) continue;

      ProductSynchronizerLogger.info(
        'Created event received; syncing remoteId=${product.id} tag=$tag',
      );
      try {
        await _downloader.downloadOne(product);
      } catch (e, st) {
        ProductSynchronizerLogger.error(
          'Failed syncing created remoteId=${product.id}',
          error: e,
          stackTrace: st,
        );
      }
  ...
  Future<void> _processUpdated(int gen, SseClientBase client) async {
    await for (final raw in client.listenJson(updatedFieldKey)) {
      if (_disposed || gen != _generation) break;
      ProductSynchronizerLogger.debug('Updated raw payload: $raw');

      ProductUpdatedEventDto? dto;
      try {
        dto = ProductUpdatedEventDto.fromJson(raw);
      } catch (e, st) {
        ProductSynchronizerLogger.error(
      ...
      ProductSynchronizerLogger.info(
        'Updated event received; syncing remoteId=${product.id} tag=$tag',
      );
      try {
        await _downloader.downloadOne(product);
      } catch (e, st) {
        ProductSynchronizerLogger.error(
          'Failed syncing updated remoteId=${product.id}',
          error: e,
          stackTrace: st,
        );
      }
  ...
  Future<void> _processDeleted(int gen, SseClientBase client) async {
    await for (final raw in client.listenJson(deletedFieldKey)) {
      if (_disposed || gen != _generation) break;
      ProductSynchronizerLogger.debug('Deleted raw payload: $raw');

      ProductDeletedEventDto? dto;
      try {
        dto = ProductDeletedEventDto.fromJson(raw);
      } catch (e, st) {
        ProductSynchronizerLogger.error(
  ...
  Future<void> _closeSseClient() async {
    ...
    if (client != null) {
      try {
        ...
        ProductSynchronizerLogger.error(
          'Error closing product SSE client (serviceName=$serviceName tag=$tag)',
          error: e,
          stackTrace: st,
        );
      }
  ...
  Future<void> _refreshFromBackend({required String reason}) async {
    ...
    try {
      ProductSynchronizerLogger.warn(
        'Refreshing products from backend due to $reason (serviceName=$serviceName tag=$tag)',
      );
